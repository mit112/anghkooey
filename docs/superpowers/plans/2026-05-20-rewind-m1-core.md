...
- **M1 exit review (Opus, 2026-05-20):** PASS. All six gate items satisfied. One CI gap found and fixed in-review: `scripts/ci.sh` did not invoke `scripts/m1-forbidden-patterns.sh`, so the tripwire wasn't actually running on PRs despite the gate text saying so. Wired in. Non-blocking observations logged for M2: (a) `CoreLog.subsystem` is a `nonisolated(unsafe) static var` and should get a one-shot `configure(subsystem:)` when M2 wires the real bundle ID; (b) long-term scheduler (`enableShortTerm == false`) remains unspecialised — default params use short-term, parity unaffected; (c) first additive M2 schema change should still add an explicit no-op migration stage to lock in the habit.
- **Next step:** open PR `m1/swiftdata-models → main`. Title: "M1 — AnghkooeyCore: SwiftData v1 schema + FSRS-6 engine + parity harness". Keep all 12 commits — do NOT squash (the contract-first story T3→T4→T5→T4-fix is portfolio signal).
